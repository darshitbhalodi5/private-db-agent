...
function validatePayload(payload) {
  ...
  if (typeof payload.prompt !== 'string' || payload.prompt.trim().length === 0) {
    return 'Payload must include prompt.';
  }
...
export async function POST(request) {
  ...
  const submissionId = `web_ai_schema_${randomUUID()}`;
  const receivedAt = new Date().toISOString();
  const agentApiBaseUrl = getAgentApiBaseUrl();
  const upstreamUrl = `${agentApiBaseUrl}/v1/ai/schema-draft`;
  ...
  try {
    ...
    if (!upstreamResponse.ok) {
      ...
      return NextResponse.json(
        {
          error: 'UPSTREAM_AI_SCHEMA_DRAFT_FAILED',
          message: 'Agent API rejected AI schema draft request.',
          submissionId,
          receivedAt,
    ...
    return NextResponse.json(
      {
        code: 'AI_SCHEMA_DRAFT_FORWARDED',
        message: 'AI schema draft request forwarded to agent API.',
        submissionId,
        receivedAt,
        upstreamStatus: upstreamResponse.status,
        upstreamBody
    ...
    return NextResponse.json(
      {
        error: 'UPSTREAM_AI_SCHEMA_DRAFT_UNREACHABLE',
