...
export async function POST(request) {
  ...
  const submissionId = `web_ai_policy_${randomUUID()}`;
  const receivedAt = new Date().toISOString();
  const agentApiBaseUrl = getAgentApiBaseUrl();
  const upstreamUrl = `${agentApiBaseUrl}/v1/ai/policy-draft`;
  ...
  try {
    ...
    if (!upstreamResponse.ok) {
      ...
      return NextResponse.json(
        {
          error: 'UPSTREAM_AI_POLICY_DRAFT_FAILED',
          message: 'Agent API rejected AI policy draft request.',
          submissionId,
          receivedAt,
    ...
    return NextResponse.json(
      {
        code: 'AI_POLICY_DRAFT_FORWARDED',
        message: 'AI policy draft request forwarded to agent API.',
        submissionId,
        receivedAt,
        upstreamStatus: upstreamResponse.status,
        upstreamBody
    ...
    return NextResponse.json(
      {
        error: 'UPSTREAM_AI_POLICY_DRAFT_UNREACHABLE',
