...
function validatePayload(payload) {
  ...
  if (typeof payload.grantId !== 'string' || payload.grantId.trim().length === 0) {
    return 'Payload must include grantId.';
  }
...
export async function POST(request) {
  ...
  const submissionId = `web_policy_revoke_${randomUUID()}`;
  const receivedAt = new Date().toISOString();
  const upstreamUrl = `${getAgentApiBaseUrl()}/v1/policy/grants/revoke`;
  ...
  try {
    ...
    if (!upstreamResponse.ok) {
      ...
      return NextResponse.json(
        {
          error: 'UPSTREAM_POLICY_GRANT_REVOKE_FAILED',
          message: 'Agent API rejected policy grant revoke payload.',
          submissionId,
          receivedAt,
    ...
    return NextResponse.json(
      {
        code: 'POLICY_GRANT_REVOKE_FORWARDED',
        message: 'Policy grant revoke payload forwarded to agent API.',
        submissionId,
        receivedAt,
        upstreamStatus: upstreamResponse.status,
        upstreamBody
    ...
    return NextResponse.json(
      {
        error: 'UPSTREAM_POLICY_GRANT_REVOKE_UNREACHABLE',
